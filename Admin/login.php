...
//cek cookie
if(isset($_COOKIE['ab']) && isset($_COOKIE['cd'])){
    $ab = $_COOKIE['ab'];
    $cd = $_COOKIE['cd'];

    //ambil username berdasarkan id
    $result = mysqli_query($conn, "SELECT username FROM user WHERE id=$ab");
    $row = mysqli_fetch_assoc($result);

    //cek cookie dan username
    if($cd === hash('sha256', $row['username'])){
        $_SESSION['login'] = true;
    }
}
    ...
    if(mysqli_num_rows($result) === 1) {
        ...
        if(password_verify($password, $row["password"])){
            ...
            if(isset($_POST['remember'])){
                //create cookie
                setcookie('ab', $row['ab'], time()+60);
                setcookie('cd', hash('sha256', $row['username']));
            }
